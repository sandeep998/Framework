...
public class MobileBase {
	...
	public AppiumDriver<MobileElement> launchMobileDriver(ExtentReports reports) throws Exception {
		...
		if(configFileObj.getProperty("app").trim().contains(".ipa")) {
			...
			if ((platformVersion != null) && (Double.valueOf(platformVersion.trim()) >= 9.3)) {
			...
			if ((platformVersion != null) && !(Double.valueOf(platformVersion.trim()) >= 7) ) {
	 ...
	 * Less than android version 7 - Used TouchAction for scrolling to find the element
	 * Greater than or equalTo android version 7 - Used UiScrollable for scrolling
	 * @param xpath
